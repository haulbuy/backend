 ...
 */
export const createOrders = async (userId: string, cartItems: any[]) => {
  ...
  const { data: insertedOrders, error: insertOrdersError } =
    await supabaseServiceClient.from("orders").insert(orders).select("id");
 ...
 */
export const processOrders = async (orderIds: string[]) => {
  ...
  for (let i = 0; i < orderIds.length; i++) {
    ...
    if (order.status === "pending") {
      ...
      if (payment.status === "paid") {
        const { data: _updatedOrder, error: updateError } =
          await supabaseServiceClient
            .from("orders")
            .update({ status: "paid" })
            .eq("id", orderIds[i])
