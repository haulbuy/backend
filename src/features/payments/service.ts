...
export const processBalancePayment = async (
    ...
    if (paymentError) {
        return {
            error:
                `Payment with ID ${paymentId} could not be processed: ${paymentError.message}`,
        };
    }
    ...
    if (balanceError) {
        return {
            error:
                `Payment with ID ${paymentId} could not be processed: ${balanceError.message}`,
        };
    }
    ...
    if (updateError) {
        return {
            error:
                `Payment with ID ${paymentId} could not be processed: ${updateError.message}`,
        };
    }
    ...
    if (paymentUpdateError) {
        return {
            error:
                `Payment with ID ${paymentId} could not be processed: ${paymentUpdateError.message}`,
        };
    }
